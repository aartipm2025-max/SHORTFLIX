...
with open("c:/SHORT FILM/data.py", "w", encoding="utf-8") as f:
    ...
    for i, item in enumerate(data):
        ...
        # 3. Replace dummy URLs
        if 'example' in item['youtube_url']:
            fallback = fallbacks[i % len(fallbacks)]
            item['youtube_url'] = f"https://www.youtube.com/watch?v={fallback}"
            item['thumbnail'] = f"https://img.youtube.com/vi/{fallback}/hqdefault.jpg"
        else:
            # We enforce high quality thumbnails but if user passed hqdefault we keep it
            pass
